...
BIOREC_FORM_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "ui_biorec.ui"))
R6_FORM_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "ui_R6.ui"))
R6_CREDENTIALS_FORM_CLASS, _ = uic.loadUiType(
    os.path.join(os.path.dirname(__file__), "ui_R6Credentials.ui"))
...
class BiorecDialog(QWidget, BIOREC_FORM_CLASS):
    def __init__(self, iface, dockwidget):
        ...
        self.cboOutputFormat.currentIndexChanged.connect(
        ...
        self.leImageFolder.setText(
            self.env.getEnvValue("biorec.r6SQLServerUserName"))
        self.r6Credentials = {
        ...
        self.butRemoveMap.setIcon(
            QIcon(
                self.pathPlugin %
                "images/removelayer.png"))
        self.butRemoveMaps.setIcon(
            QIcon(
                self.pathPlugin %
                "images/removelayers.png"))
        self.butHelp.setIcon(QIcon(self.pathPlugin % "images/info.png"))
        self.butGithub.setIcon(QIcon(self.pathPlugin % "images/github.png"))
        self.butSaveImage.setIcon(
            QIcon(
                self.pathPlugin %
                "images/saveimage.png"))
        self.butShowAll.setIcon(
            QIcon(
                self.pathPlugin %
                "images/layershow.png"))
        self.butHideAll.setIcon(
            QIcon(
                self.pathPlugin %
                "images/layerhide.png"))
        ...
        self.mlcbSourceLayer.setFilters(
            QgsMapLayerProxyModel.PointLayer | QgsMapLayerProxyModel.NoGeometry)
        self.mlcbTaxonMetaDataLayer.setFilters(
    ...
    def ChkR6Setup(self):
        ...
        if platform.system() == 'Windows':
            try:
                # r6Key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE,
                # r'SOFTWARE\WOW6432Node\Dropbox', 0, #For testing on non-R6
                # machine
                r6Key = winreg.OpenKey(
                    winreg.HKEY_LOCAL_MACHINE,
                    r'SOFTWARE\WOW6432Node\Dorset Software',
                    0,
                    (winreg.KEY_WOW64_64KEY + winreg.KEY_READ))
            except BaseException:
                # There was some sort of error trying to retrieve the R6
                # registry key
                r6Key = None

            if r6Key:
                # Registry R6 keys found, retrieve servername and user it to
                # build trusted connection string
                subname = winreg.EnumKey(r6Key, 0)
                subkey = winreg.OpenKey(
                    r6Key, subname, 0, (winreg.KEY_WOW64_64KEY + winreg.KEY_READ))
                # self.r6Credentials["server"] = winreg.QueryValueEx(subkey,
                # "Version")[0] #For testing on non-R6 machine
                self.r6Credentials["server"] = winreg.QueryValueEx(
                    subkey, "Server Name")[0]
                con_str = "DRIVER={{SQL Server}};SERVER={0};DATABASE=NBNData;Trusted_Connection=yes;".format(
                    '{' + self.r6Credentials["server"] + '}')
                # Attempt to open DB with trusted connection string
                self.waitMessage(
                    "Connecting to DB",
                    "attempting to connect to R6 DB with trusted connection...")
            ...
            if not dbOpen:
                # Prompt user for R6 servername (already initialised if r6Key
                # found), username and password
                if self.getR6Credentials():
                    con_str = 'DRIVER={{SQL Server}};SERVER={0};Database=NBNData;UID={1};PWD={2};'.format(
                        '{' + self.r6Credentials["server"] + '}', self.r6Credentials["user"], self.r6Credentials["pword"])
                    # Attempt connection with supplied credentials
                    self.waitMessage(
                        "Connecting to DB",
                        "attempting to connect to R6 DB with supplied credentials...")
                    db.setDatabaseName(con_str)
                    self.waitMessage()
                    if db.open():
                        dbOpen = True
    ...
    def showEvent(self, ev):
        ...
        self.env = envmanager.envManager()
        self.leImageFolder.setText(
            self.env.getEnvValue("biorec.atlasimagefolder"))
        ...
        if self.csvLayer is not None:
            ...
            if self.csvLayer.geometryType() == 3 or self.csvLayer.geometryType() == 4:
                ...
                if self.fcbGridRefCol.currentField() != "":
                    ...
                    # When grid references are used for input, Input CRS is not
                    # set
                    ...
                    self.pswOutputCRS.setEnabled(
                        not self.rbOutCrsInput.isChecked())
    ...
    def enableDisableXY(self):
        ...
        if (self.fcbXCol.currentField() !=
                "" or self.fcbYCol.currentField() != "") and not self.blockGR:
            if self.fcbGridRefCol.currentField() != "":
                self.fcbGridRefCol.setField("not set")
    ...
    def browseImageFolder(self):
        ...
        folderName = dlg.getExistingDirectory(
            self, "Browse for image folder", strInitPath)
        if folderName:
    ...
    def browseStyleFile(self):
        ...
        fileName = dlg.getOpenFileName(
            self,
            "Browse for style file",
            strInitPath,
            "QML Style Files (*.qml)")[0]
        if fileName:
    ...
    def setCSV(self, nbnFile):
        ...
        if nbnFile is None:
            ...
            fileName = dlg.getOpenFileName(
                self,
                "Browse for biological record file",
                strInitPath,
                "Record Files (*.csv)")[0]
        ...
        if fileName != "":
            ...
            self.loadCsv(fileName, (nbnFile is not None))
    ...
    def listTaxa(self, suppressMessage=False):
        ...
        self.waitMessage("Building taxon tree",
                         "can take a minute or so for very large files...")
        ...
        # The technique for sorting dicts came from
    ...
    def loadCsv(self, fileName, isNBNCSV):
        ...
        try:
            lyr = QgsVectorLayer(
                uri, os.path.basename(fileName), "delimitedtext")
            # Add to registry
        ...
        if self.csvLayer != self.mlcbSourceLayer.currentLayer():
            ...
            if self.csvLayer is not None:
                ...
                if self.isNBNCSV:
                    ...
                    for colGridRef in self.env.getEnvValues(
                            "biorec.gridrefcol"):
                        index = 1
                        for field in self.csvLayer.dataProvider().fields():
                ...
                for colAbundance in self.env.getEnvValues(
                        "biorec.abundancecol"):
                    index = 1
                    for field in self.csvLayer.dataProvider().fields():
    ...
    def MapRecords(self):
        ...
        # listed in map registry. This accounts for people removing layers
        ...
        if self.pswOutputCRS.crs().authid() == "":
            self.iface.messageBar().pushMessage(
                "Info", "You must specify an output CRS", level=Qgis.Info)
            return

        # Return if grid ref field set and user-defined grid is set
        if self.fcbGridRefCol.currentField(
        ) != "" and self.cboMapType.currentText().startswith("User-defined"):
            self.infoMessage(
                "Can't set a user-defined grid size when using grid references as input")
            return

        # Return if output is records as grid squares but grid ref column not
        # set
        if self.cboMapType.currentText(
        ) == "Records as grid squares" and self.fcbGridRefCol.currentField() == "":
            self.infoMessage(
                "'Records as grid squares' only available for input as grid references")
            return

        # Return if no grid reference or X & Y fields selected - but only for
        # layers without geometry
        if self.csvLayer.geometryType() == 3 or self.csvLayer.geometryType() == 4:

            self.fcbGridRefCol.currentField() == ""

            if self.fcbGridRefCol.currentField() == "" and (
                    self.fcbXCol.currentField() == "" or self.fcbYCol.currentField() == ""):
                self.iface.messageBar().pushMessage(
                    "Info",
                    "You must select either a Grid Ref column or both X and Y columns for CSV layers",
                    level=Qgis.Info)
                return
        ...
        if self.fcbGridRefCol.currentField(
            ...
            self.iface.messageBar().pushMessage(
                "Info",
                "You cannot specify a user-defined grid with input of grid references",
                level=Qgis.Info)
            return
        ...
        if self.fcbXCol.currentField() != "" and self.pswInputCRS.crs().authid() == "":
            self.iface.messageBar().pushMessage(
                "Info",
                "You must specify an input CRS if specifying X and Y columns",
                level=Qgis.Info)
            return
        ...
        if self.fcbXCol.currentField() != "" and self.pswOutputCRS.crs().authid() == "":
            self.iface.messageBar().pushMessage(
                "Info",
                "You must specify an output CRS if specifying X and Y columns",
                level=Qgis.Info)
            return

        # Return if user-defined atlas selected, but no grid size
        if self.cboMapType.currentText().startswith(
                "User-defined") and self.dsbGridSize.value() == 0:
            self.iface.messageBar().pushMessage(
                "Info",
                "You must specify a grid size if specifying a user-defined atlas",
                level=Qgis.Info)
            return

        # Return if a grid option selected, but the output CRS is not Irish or
        # British Grid (unless input is grid references)
        if self.fcbGridRefCol.currentField() == "" and not self.cboMapType.currentText().startswith(
                "User-defined") and not self.cboMapType.currentText() == "Records as points":
            if self.pswOutputCRS.crs().authid(
            ) != "EPSG:27700" and self.pswOutputCRS.crs().authid() != "EPSG:29903":
                self.infoMessage(
                    "Only points or user-defined grid can be use for output CRS that is not either Irish or British Grid (EPSG:29903 or EPSG:2770) where input is not grid references")
                return
    ...
    def batchGeneration(self):
        ...
        if len(self.layers) == 0:
            self.infoMessage(
                "There are no temporary biorec layers to work with.")
            return
        ...
        if not os.path.exists(imgFolder):
            if not self.folderError:
                self.iface.messageBar().pushMessage(
                    "Error", "Output folder '%s' does not exist." %
                    (imgFolder), level=Qgis.Warning)
                return
        format = self.cboOutputFormat.currentText()
        if (format == "GeoJSON") or (format == "Shapefile"):
    ...
    def batchPrintComposer(self):
        ...
        if iLayouDesigners > 1:
            self.warningMessage(
                "You have more than one layout open. Make sure that only one print layout (composer) is open.")
            return
    ...
    def createMapLayer(self, selectedTaxa):
        ...
        layer = bioreclayer.biorecLayer(
            self.iface, self.csvLayer, self.pteLog, self.progBatch)
        ...
        layer.setCrs(
            self.pswInputCRS.crs().authid(),
            self.pswOutputCRS.crs().authid())
        ...
        if self.cbApplyStyle.isChecked():
            ...
            if self.cboMapType.currentIndex() == 0:
                self.iface.messageBar().pushMessage(
                    "Warning",
                    "Applied style file to points layer. Points will not be visible if no style for points defined.",
                    level=Qgis.Warning)
        ...
        layer.createMapLayer(
            self.cboMapType.currentText(),
            self.cboSymbol.currentText(),
            styleFile)
        self.waitMessage()
    ...
    def saveTempToLayer(self, layer, format):
        ...
        error = QgsVectorFileWriter.writeAsVectorFormatV3(
            layer.getVectorLayer(), filePath, tc, vOptions)

        if error[0] != QgsVectorFileWriter.NoError:
            self.iface.messageBar().pushMessage(
                "Error", "Layer generation error: %s" %
                (error), level=Qgis.Warning)
            return
    ...
    def saveMapImage(self, image, name):
        ...
        try:
            ...
            if not self.imageError:
                e = sys.exc_info()[0]
                self.iface.messageBar().pushMessage(
                    "Error", "Image generation error: %s" %
                    (err), level=Qgis.Warning)
    ...
    def saveComposerImage(self, name, layers):
        ...
        # For each text item, replace the tokens, e.g. #name#, with
        # corresponding text.

        # Get all text items from the layout.
        # Item.scene() ensures that the item is being used (not deleted)
        textItems = [item for item in l.items() if type(
            item).__name__ == "QgsLayoutItemLabel" and item.scene()]

        # Save the original text item values so that they can be reset
        # afterwards
        originalText = []
        ...
        metaOutFileName = ''
        if self.cbTaxonMetaData.isChecked(
        ) and self.mlcbTaxonMetaDataLayer.currentLayer() is not None:
            metaLayer = self.mlcbTaxonMetaDataLayer.currentLayer()
            # The regular expression (~ comparison) allows for leading and
            # trailing white space on the taxa
        ...
        try:
            [item for item in l.items() if type(item).__name__ ==
             "QgsLayoutItemMap"][0].setLayers(layers)
            ...
            if format == "Composer image":
                ...
                res = le.exportToSvg(
                    imgFolder + os.path.sep + outName + ".svg", s)
        except BaseException as err:
            if not self.imageError:
                e = sys.exc_info()[0]
                self.iface.messageBar().pushMessage(
...
class R6CredentialsDialog(QDialog):
    def __init__(self, initCredentials):
        ...
        self.ui = R6_CREDENTIALS_FORM_CLASS()
    ...
    def textChanged(self):
        if self.ui.leR6Server.text().strip() and self.ui.leR6User.text(
        ).strip() and self.ui.leR6Password.text().strip():

            self.ui.bbDialogButtons.button(
...
class R6Dialog(QDialog):
    ...
    def getR6Data(self):
        ...
        for column in range(exportQSqlQueryModel.columnCount()):
            column_names.append(
                str(exportQSqlQueryModel.headerData(column, Qt.Horizontal)))
        ...
        if len(list_of_rows) > 0:
            ...
            csvMemoryLayer = QgsVectorLayer(
                'None', 'R6 ' + self.ui.cmbSpToMap.currentText(), 'memory')
            ...
            for x in range(colcount):
                if query.record().field(x).type() == 10:
                    dataProvider.addAttributes(
                        [QgsField(query.record().fieldName(x), QVariant.String)])
                else:  # ==3
                    dataProvider.addAttributes(
                        [QgsField(query.record().fieldName(x), QVariant.Int)])
            ...
            for x in list_of_rows:
                # writer.writerow(x)
                fet = QgsFeature()
                # For some reason row x has an extra value of None at the end
                # so trim this off (otherwise feature addition fails)
                fet.setAttributes(x[:-1])
                fets.append(fet)
