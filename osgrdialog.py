...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), "ui_osgr.ui"))


class OsgrDialog(QWidget, FORM_CLASS):
    ...
    def boxDragged(self):
        ...
        if self.rbOutCrsBritish.isChecked() and self.canvas.mapSettings(
        ).destinationCrs().authid() != "EPSG:27700":
            transGrid = QgsCoordinateTransform(
        ...
        if transGrid is None:
            ...
            point = QgsPointXY(
                float(
                    self.dragTool.xMinimum), float(
                    self.dragTool.yMinimum))
            gridPoint = transGrid.transform(point)
            xMin = gridPoint.x()
            yMin = gridPoint.y()
            point = QgsPointXY(
                float(
                    self.dragTool.xMaximum), float(
                    self.dragTool.yMaximum))
            gridPoint = transGrid.transform(point)
            xMax = gridPoint.x()
            yMax = gridPoint.y()
    ...
    def GridPoly(self):
        ...
        if layer is None:
            self.iface.messageBar().pushMessage(
                "Info", "No active layer is in layers panel.", level=Qgis.Info)
            return
        ...
        if layer.type() == QgsMapLayer.VectorLayer:
            ...
            if len(selectedFeatures) > 0:
                ...
                self.iface.messageBar().pushMessage(
                    "Info", "There are no features selected in the active layer.", level=Qgis.Info)
        else:
            self.iface.messageBar().pushMessage(
                "Info", "The active layer is not a vector layer.", level=Qgis.Info)
    ...
    def GenerateSquares(
            ...
            yMax,
            selectedGeometries=[],
            selectedFeaturesCrs=None):

        # The bounds passed into this function should be expressed in the CRS
        # of grid type - British, Irish or other (map canvas)
        if selectedFeaturesCrs is not None:
            crsAuthId = selectedFeaturesCrs.authid()
        else:
            crsAuthId = "none"
        self.logMessage(str(xMin) +
                        " " +
                        str(yMin) +
                        " " +
                        str(xMax) +
        ...
        iSquares = (((xMax - llx) // self.precision + 1)
                    * ((yMax - lly) // self.precision + 1))
        if iSquares > 1000:
            ...
            ret = QMessageBox.warning(
                self,
                "Warning",
                "Large area specified relative to the precision (" +
                precisionText +
                "). This could generate up to " +
                str(
                    int(iSquares)) +
        ...
        self.osgrLayer.GenerateSquares(
            xMin,
            yMin,
            xMax,
            yMax,
    ...
    def locateOnGR(self, zoom):
        ...
        if res[0] == 0:
            self.iface.messageBar().pushMessage(
                "Warning", res[3], level=Qgis.Warning)
        else:
            precision = res[2]
    ...
    def mapToolClicked(self, tool):
        # We get here whenever the mapTool changes - useful for un-setting
        # custom mapTool buttons
        try:
            click_tool = getattr(self, 'clickTool', None)
            drag_tool = getattr(self, 'dragTool', None)

            if (click_tool is not None and tool != click_tool):
                self.cbGROnClick.setChecked(False)
                self.cbGRShowSquare.setChecked(False)
                self.cbGROnClick.setChecked(False)
                # pass
            if (drag_tool is not None and tool != drag_tool):
                self.butGridTool.setChecked(False)
                # self.butGridTool.setIcon(QIcon( self.pathPlugin % "images/osgr.png" ))
        except AttributeError:
            # During plugin reload an old partially-unloaded instance can still
            # receive mapToolSet.
            return
    ...
    def canvasMouseMove(self, point):
        try:
            ...
            # Catches an error on map canvas event handler that can happen if
            # the tool unloaded?
            pass

    def displayOSGR(self, point):

        # Transform the map canvas points if maps canvas is not set to British
        # or Irish grid
        if self.grType == "os" and self.canvas.mapSettings(
        ).destinationCrs().authid() != "EPSG:27700":
            transGrid = QgsCoordinateTransform(
                self.canvas.mapSettings().destinationCrs(),
                QgsCoordinateReferenceSystem("EPSG:27700"),
        ...
        if (self.cbGRShowSquare.isChecked() and self.grPrecision >
                0) or self.cbGRShowPoint.isChecked():
            x0 = (transPoint.x() // self.grPrecision) * self.grPrecision
            y0 = (transPoint.y() // self.grPrecision) * self.grPrecision
            centrePoint = QgsGeometry.fromPointXY(
                QgsPointXY(
                    x0 + self.grPrecision / 2,
                    y0 + self.grPrecision / 2))
            points = [
                QgsPoint(
                    x0,
                    y0),
            ...
            # Transform the transformed geom back to canvas geom if maps canvas
            # is not set to British or Irish grid
            if self.grType == "os" and self.canvas.mapSettings(
            ).destinationCrs().authid() != "EPSG:27700":
                transCanvas = QgsCoordinateTransform(
                    QgsCoordinateReferenceSystem("EPSG:27700"),
                    self.canvas.mapSettings().destinationCrs(),
                    QgsProject.instance())
            elif self.grType == "irish" and self.canvas.mapSettings().destinationCrs().authid() != "EPSG:29903":
    ...
    def __unload(self):
        self.clearMapGraphics(True, True)
        try:
            self.canvas.mapToolSet.disconnect(self.mapToolClicked)
        except BaseException:
            pass
        try:
            self.canvas.xyCoordinates.disconnect(self.canvasMouseMove)
        except BaseException:
            pass
