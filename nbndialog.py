...
class NBNDialog(QWidget, FORM_CLASS):
    ...
    def __init__(self, iface, dockwidget):
        ...
        self.pbUncheckAllSpeciesList.clicked.connect(
        ...
        self.pbBrowseNbnOutputFolder.clicked.connect(
            self.browseNbnOutputFolder)
        ...
        self.cbShowAllMetadata.stateChanged.connect(
        ...
        self.transformCrs = QgsCoordinateTransform(
        ...
        self.butClearLast.setIcon(
            QIcon(
                self.pathPlugin %
                "images/removelayer.png"))
        self.butClear.setIcon(
            QIcon(
                self.pathPlugin %
                "images/removelayers.png"))
        self.pbSpeciesWMS.setIcon(
            QIcon(
                self.pathPlugin %
                "images/nbngridmap.png"))
        self.pbDownload.setIcon(
            QIcon(
                self.pathPlugin %
                "images/nbndownload.png"))
        self.butTaxonSearch.setIcon(
            QIcon(
                self.pathPlugin %
                "images/speciesinventory.png"))
        self.pbBuffer.setIcon(QIcon(self.pathPlugin % "images/buffer.png"))
        self.pbClearLastBuffer.setIcon(
            QIcon(
                self.pathPlugin %
                "images/bufferclear.png"))
        ...
        self.infoFile = os.path.join(
            os.path.dirname(__file__), "infoNBNTool.txt")
        ...
        self.cboDownloadReason.currentIndexChanged.connect(
            self.downloadReasonChanged)

    def downloadReasonChanged(self):
        if self.cboDownloadReason.currentData() != -99:
            self.cboDownloadReason.setToolTip(
                self.reasonTips[self.cboDownloadReason.currentData()])
        else:
            self.cboDownloadReason.setToolTip(
                "Specify a reason for NBN download")
    ...
    def browseNbnOutputFolder(self):
        ...
        folderName = dlg.getExistingDirectory(
            self, "Browse for NBN download folder", strInitPath)
        if folderName:
    ...
    def showEvent(self, ev):
        ...
        self.leNbnOutputFolder.setText(
            self.env.getEnvValue("nbn.downloadfolder"))
    ...
    def datasetSelectionChanged(self):
        ...
        if iChecked == 0:
            ...
            self.lblDatasetFilter.setText(
                str(iChecked) + " datasets selected.")
    ...
    def speciesListSelectionChanged(self):
        ...
        if iChecked == 0:
            ...
            self.lblSpeciesListFilter.setText(
                str(iChecked) + " species lists selected.")
    ...
    def refreshProviders(self):
        ...
        datafile = self.pathPlugin % (
            "NBNCache%s%s" % (os.path.sep, "providers.json"))
    ...
    def refreshSpeciesLists(self):
        ...
        # Write the json data to a file
        datafile = self.pathPlugin % (
            "NBNCache%s%s" % (os.path.sep, "species-lists.json"))
        with open(datafile, 'w') as jsonfile:
            jsonfile.write(responseText)
        ...
        if twi.text(2) == "provider" and twi.childCount() == 0:
            ...
            for dataResource in jsonData["dataResources"]:
                QgsMessageLog.logMessage(
                    "dataset: " + dataResource["name"], "NBN Tool")
    ...
    def readProviderFile(self):
        ...
        try:
            ...
            self.warningMessage(
                "NBN Atlas data providers file failed to load. Use the refresh button to generate a new one.")
            return

        for jDataset in jsonData:

            # Create a new top level tree item for the dataset provider
            # organisation
    ...
    def readSpeciesListFile(self):
        ...
        try:
            ...
            self.warningMessage(
                "NBN Atlas data species list file failed to load. Use the refresh button to generate a new one.")
            return

        # List types are initialised to known list types so that order can be
        # specified
        knownListTypes = [
            "CONSERVATION_LIST",
            "SENSITIVE_LIST",
        ...
        for speciesList in readListTypes:
            if speciesList not in listTypes:
                listTypes.append(speciesList)
    ...
    def taxonSearch(self):
        ...
        if self.leTaxonSearch.text() == "":
            self.iface.messageBar().pushMessage(
                "No search term specified.", level=Qgis.Info)
            return

        url = 'https://species-ws.nbnatlas.org/search?q=' + \
            self.leTaxonSearch.text() + '&pageSize=50&fq=taxonomicStatus:accepted'
        ...
        for jTaxon in jResponseList:
            ...
            if jTaxon["name"].lower() == self.leTaxonSearch.text().lower(
            ) or jTaxon["commonNameSingle"].lower() == self.leTaxonSearch.text().lower():
            ...
            if 'commonNameSingle' in jTaxon and jTaxon["commonNameSingle"] != "":
                tName = jTaxon["name"] + \
                    " (" + jTaxon["commonNameSingle"] + ")"
            else:
                tName = jTaxon["name"]

            if tKingdom != "" and tKingdom not in (treeNodes.keys()):
            ...
            if tPhylum != "" and tPhylum not in (treeNodes.keys()):
                # Create a new top level tree item for the taxon group
            ...
            if tClass != "" and tClass not in (treeNodes.keys()):
                # Create a new top level tree item for the taxon group
            ...
            if tOrder != "" and tOrder not in (treeNodes.keys()):
                # Create a new top level tree item for the taxon group
            ...
            if tFamily != "" and tFamily not in (treeNodes.keys()):
                # Create a new top level tree item for the taxon group
            ...
            if tGenus != "" and tGenus not in (treeNodes.keys()):
                # Create a new top level tree item for the taxon group
    ...
    def makeFilterQuery(self):
        ...
        if startYear is not None and endYear is not None:
            if startYear > endYear:
                self.infoMessage(
                    "End year, if specified, must come after start year (or be equal to it), if specified.")
                return None
    ...
    def WMSFetchSpecies(self):
        ...
        if tvks is None:
            self.iface.messageBar().pushMessage(
                "Info",
                "You must first specify a taxon filter for a WMS map layer.",
                level=Qgis.Info)
            return
        ...
        if len(tvks) > 1:
            self.iface.messageBar().pushMessage(
                "Info",
                "You must only specify one taxon filter for a WMS map layer.",
                level=Qgis.Info)
            return

        # Check that taxon rank is allowable given current limitations of Atlas
        # WMS
        allowedRanks = [
            "kingdom",
            "phylum",
        ...
        if taxonRank not in allowedRanks:
            self.infoMessage(
                ("The taxonomic rank - " + taxonRank + " - of " + taxonName + " is not one "
    ...
    def checkTransform(self):
        ...
        try:
            # Not sure why, but this sometimes crashes after another failure in
            # module
            if self.canvasCrs != self.canvas.mapSettings().destinationCrs():
                self.canvasCrs = self.canvas.mapSettings().destinationCrs()
                self.transformCrs = QgsCoordinateTransform(
                    self.canvasCrs, self.osgbCrs, QgsProject.instance())
        except BaseException:
            pass
    ...
    def mapExtentsChanged(self):
        ...
        if self.canvasCrs != self.osgbCrs:
            mapWidth = self.transformCrs.transformBoundingBox(
                rectExtent).width()
        else:
                    ...
                    if mapWidth < 15000:
                        ...
                        if rlayer.name().endswith("auto"):
                            ...
                            QgsProject.instance().layerTreeRoot().findLayer(
                                rlayer.id()).setItemVisibilityChecked(True)
                        else:
                            # self.iface.legendInterface().setLayerVisible(rlayer, False)
                            QgsProject.instance().layerTreeRoot().findLayer(
                                rlayer.id()).setItemVisibilityChecked(False)
    ...
    def taxonDetails(self, guid):
        ...
        if res is None:
            self.iface.messageBar().pushMessage(
                "Info",
                "No species found for TVK!.",
                level=Qgis.Info)  # Should never happen
            return
    ...
    def downloadNBNObservations(self):
        ...
        if self.cboDownloadReason.currentData() == -99:
            self.iface.messageBar().pushMessage(
                "Info",
                "You must specify a download reason from the drop-down list.",
                level=Qgis.Info)
            return
        ...
        if self.leNbnOutputFolder.text().strip() == "":
            self.iface.messageBar().pushMessage(
                "Info",
                "You must specify an output folder for the downloaded data.",
                level=Qgis.Info)
            return

        if not os.path.exists(self.leNbnOutputFolder.text()):
            self.iface.messageBar().pushMessage(
                "Info", "The specified output folder cannot be found.", level=Qgis.Info)
            return
        ...
        try:
            ...
            self.iface.messageBar().pushMessage(
                "Info", "The specified output folder is not writeable.", level=Qgis.Info)
            return

        # A taxon or dataset filter must be specified
        if self.getSelectedTVK() is None and self.selectedSpeciesLists(
        ) is None and not self.datasetsSelected() and self.getSelectedFeatureWKT() is None:
            self.iface.messageBar().pushMessage(
                "Info",
                "First specify one or more of taxon, dataset, species list or polygon filters.",
                level=Qgis.Info)
            return
        ...
        fileName = os.path.join(
            self.leNbnOutputFolder.text(),
            self.getLayerName() + ".csv")
        ...
        endpoint = 'https://records-ws.nbnatlas.org/occurrences/index/download?reasonTypeId=' + \
            str(self.cboDownloadReason.currentData()) + '&qa=none&q=*:*' + fq
    ...
    def checkFilters(self):
        ...
        # Species List
        self.cbIndSpeciesList.setChecked(
            self.selectedSpeciesLists() is not None)
        # polygon
    ...
    def downLoadFinished(self, downloadInfo):
        ...
        if error != QNetworkReply.NoError:
            ...
            self.iface.messageBar().pushMessage(
                ...
                (error, reply.errorString()), level=Qgis.Warning)
        ...
        try:
            ...
            twiDownload.child(0).setIcon(
                0, QIcon(
                    self.pathPlugin %
                    "images/citations.png"))
            twiDownload.child(0).setToolTip(0, fileNoExt + '-citation.csv')
            twiHeadings = twiDownload.addChild(
                QTreeWidgetItem(['Column metadata'], 0))
            twiDownload.child(1).setIcon(
                0, QIcon(
                    self.pathPlugin %
                    "images/headings.png"))
            ...
            self.iface.messageBar().pushMessage(
                "Error", "Failed to write output file. Error: %s" %
                (str(e)), level=Qgis.Warning)
            twiDownload.setIcon(0, QIcon(self.pathPlugin % "images/cross.png"))
        finally:
            reply.deleteLater()
    ...
    def downloadClicked(self, twi, iCol):
        ...
        if filePath[-13:] == '-citation.csv':
            metadataInfo += (
                "details the organisations that provided the records you have downloaded. "
            ...
            self.defaultColumns = ["Name", "Citation",
                                   "Rights", "Number of Records in Download"]
        else:
    ...
    def restRequest(
        ...
        # and
        # http://nullege.com/codes/search/PyQt4.QtNetwork.QNetworkAccessManager
        ...
        if callType == "download":
            ...
            try:
                reply.finished.connect(
                    lambda: self.downLoadFinished(downloadInfo))
            except BaseException:
                e = sys.exc_info()[0]
                QgsMessageLog.logMessage("error generated lambda", "NBN Tool")
                downloadInfo["twi"].setIcon(
                    0, QIcon(
                        self.pathPlugin %
                        "images/cross.png"))
                self.iface.messageBar().pushMessage(
                    "Error", "NBN web service error. Error: %s" %
                    (str(e)), level=Qgis.Warning)
            return
