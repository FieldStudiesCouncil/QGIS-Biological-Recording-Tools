...
class TomBio:
    ...
    def initGui(self):
        ...
        icon_path = os.path.join(
            os.path.dirname(__file__),
            'images/maptaxa.png')
        self.actionBiorec = QAction(
            QIcon(icon_path),
            u"Biological Records Tool",
            self.iface.mainWindow())
        ...
        self.actionHelp = QAction(
            QIcon(icon_path),
            u"Help and Info on FSC QGIS plugin",
            self.iface.mainWindow())
        ...
        # Environment Options
        self.actionEnv = QAction(
            u"Environment Options",
            self.iface.mainWindow())
    ...
    def showOsgrDialog(self):
        if self.dwOsgr is None:
            self.dwOsgr = custDockWidget(
                "FSC - OSGR Tool", self.iface.mainWindow())
            self.guiOsgr = osgrdialog.OsgrDialog(self.iface, self.dwOsgr)
            self.dwOsgr.setWidget(self.guiOsgr)
            self.iface.addDockWidget(
                Qt.DockWidgetArea.LeftDockWidgetArea, self.dwOsgr)
    ...
    def showNbnDialog(self):
        if self.dwNbn is None:
            self.dwNbn = custDockWidget(
                "FSC - NBN Atlas Tool", self.iface.mainWindow())
            self.guiNbn = nbndialog.NBNDialog(self.iface, self.dwNbn)
            self.dwNbn.setWidget(self.guiNbn)
            self.iface.addDockWidget(
                Qt.DockWidgetArea.RightDockWidgetArea, self.dwNbn)
    ...
    def showMapmashupDialog(self):
        if self.dwMapmashup is None:
            self.dwMapmashup = custDockWidget(
                "FSC - Map Mashup Tool", self.iface.mainWindow())
            self.guiMapmashup = mapmashupdialog.MapmashupDialog(
                self.iface, self.dwMapmashup)
    ...
    def showBiorecDialog(self):
        if self.dwBiorec is None:
            self.dwBiorec = custDockWidget(
                "FSC - Biological Records Tool",
                self.iface.mainWindow())
            self.guiBiorec = biorecdialog.BiorecDialog(
                self.iface, self.dwBiorec)
            self.dwBiorec.setWidget(self.guiBiorec)
            self.iface.addDockWidget(
                Qt.DockWidgetArea.RightDockWidgetArea, self.dwBiorec)
        else:
    ...
    def unload(self):
        # Disconnect OSGR canvas signals before plugin teardown to avoid stale
        # callbacks on reload.
        if hasattr(self, 'guiOsgr') and self.guiOsgr is not None:
            try:
                self.guiOsgr.unload()
